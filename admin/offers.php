...
<?php if($showForm):?>
<div class="card">
  <div class="card-header"><span class="card-title"><?=$editing?'Edit Offer':'Add Offer'?></span><a href="offers.php" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-left"></i> Back</a></div>
  <form method="POST" enctype="multipart/form-data" class="admin-form">
    ...
    <?php endif;?>
    <div class="form-row">
      <div class="form-group"><label>Title (English)</label><input type="text" name="title_en" value="<?=e($editing['title_en']??'')?>" required></div>
      <div class="form-group"><label>Title (Arabic)</label><input type="text" name="title_ar" value="<?=e($editing['title_ar']??'')?>" required></div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
      <div class="form-group"><label>Discount Type</label><select name="discount_type"><option value="percentage" <?=($editing['discount_type']??'')==='percentage'?'selected':''?>>Percentage (%)</option><option value="fixed" <?=($editing['discount_type']??'')==='fixed'?'selected':''?>>Fixed Amount ($)</option></select></div>
      <div class="form-group"><label>Discount Value</label><input type="number" name="discount_value" value="<?=$editing['discount_value']??''?>" step="0.01" required></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Promo Code</label><input type="text" name="promo_code" value="<?=e($editing['promo_code']??'')?>" style="text-transform:uppercase" placeholder="e.g. SUMMER20"></div>
      <div class="form-group"><label>Minimum Nights</label><input type="number" name="min_nights" value="<?=$editing['min_nights']??1?>"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Valid From</label><input type="date" name="valid_from" value="<?=$editing['valid_from']??''?>"></div>
      <div class="form-group"><label>Valid To</label><input type="date" name="valid_to" value="<?=$editing['valid_to']??''?>"></div>
    </div>
    <div class="form-group"><label>Image</label><?php if(!empty($editing['image'])):?><img src="<?=SITE_URL?>/<?=e($editing['image'])?>" style="width:100px;height:70px;object-fit:cover;border-radius:6px;margin-bottom:8px;display:block;"><?php endif;?><input type="file" name="image" accept="image/*"></div>
    <div style="display:flex;gap:20px;margin-bottom:16px;">
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;"><input type="checkbox" name="is_active" value="1" <?=($editing['is_active']??1)?'checked':''?>> Active</label>
...
<?php else:?>
<div class="card">
  <div class="card-header"><span class="card-title">All Offers (<?=count($offers)?>)</span><a href="?action=add" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Add Offer</a></div>
  <div class="table-wrap"><table>
    <thead><tr><th>Image</th><th>Title</th><th>Discount</th><th>Promo Code</th><th>Valid Until</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>
      ...
      <tr>
        ...
        <td><small><?=$o['valid_to']?formatDate($o['valid_to']):'No expiry'?></small></td>
        <td><?=$o['is_active']?'<span class="badge badge-success">Active</span>':'<span class="badge badge-secondary">Inactive</span>'?> <?=$o['is_featured']?'<span class="badge badge-warning">Featured</span>':''?></td>
